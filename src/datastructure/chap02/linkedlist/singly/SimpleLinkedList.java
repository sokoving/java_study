...
//        System.out.printf("헤더가 새롭게 감시할 첫번째 노드의 주소: %s\n", header.getFirstNode());
    ...
    // 리스트의 첫번째 노드 삭제(메서드의 결과로 삭제한 노드를 리턴해 주는 게 관례)
    public Node removeFirst() {
        // 빈 리스트인지 확인
        if (!header.isEmpty()) {
            // 삭제 대상 노드를 키핑
            Node tempNode = header.getFirstNode();
            // 삭제 대상 노드에서 다음 노드 주소 가져오기
            ...
            return null; // 빈 리스트면 null 리턴(삭제할 것 없다)
        }
    }
    ...
    public void insert(int data) {
        ...
        // 첫 번째 노드를 가져와서 현재 노드로 설정   >> 반복문 탈출 조건
        Node current = header.getFirstNode();
        // 바로 이전 노드를 가져오기               >> 주소 바꿀 대상 (current와 prev는 따라다닌다)
        Node prev = null;

        // 삽입 위치 탐색
        // 빈 리스트가 아님 && 삽입할 데이터가 현재 노드의 데이터보다 작아질 때까지
    ...
    public Node remove(int data) {
        ...
        // 삭제 위치 탐색
        // 빈 리스트가 아님 && 삽입할 데이터가 현재 노드의 데이터와 다를 동안 반복
        while (current != null && data != current.getData()) {
            // 현재 노드를 이전 노드에 백업
            prev = current;
            // 다음 노드를 현재 노드로 재설정
